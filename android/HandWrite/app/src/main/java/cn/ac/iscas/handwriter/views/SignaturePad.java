package cn.ac.iscas.handwriter.views;
...
import cn.ac.iscas.handwriter.R;
import cn.ac.iscas.handwriter.utils.Bezier;
import cn.ac.iscas.handwriter.utils.ControlTimedPoints;
import cn.ac.iscas.handwriter.utils.SvgBuilder;
import cn.ac.iscas.handwriter.utils.TimedPoint;
import cn.ac.iscas.handwriter.view.ViewCompat;
import cn.ac.iscas.handwriter.view.ViewTreeObserverCompat;
...
public class SignaturePad extends View {
    ...
    // MotionEvent record set.
    private SparseArray<MotionEventRecorder> mMotionEventRecord = new SparseArray<>();
    // record index.
    private int mMotionEventRecordIndex = 0;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                addPoint(getNewPoint(eventX, eventY));
                // add motion event record.
                mMotionEventRecord.put(mMotionEventRecordIndex++,
                        new MotionEventRecorder(event.getEventTime(),
                                event.getX(),
                                event.getY(),
                                event.getPressure()));
                break;
    ...
    public SparseArray<MotionEventRecorder> getMotionEventRecord() {
        mMotionEventRecordIndex = 0;
        // SparseArray<MotionEventRecorder> tempRecord = this.mMotionEventRecord;
        // this.mMotionEventRecord = new SparseArray<>();
        return this.mMotionEventRecord;
    }
    ...
    public class MotionEventRecorder {
        ...
        public MotionEventRecorder(long time, float x, float y, float z) {
            this.time = time;
            this.x = x;
            this.y = y;
            this.z = z;
